...
Создан уникальный индекс по Content.name для строк с category_id = 0. (Возможность добавления файлов с одинаковым именем убрана во избежание проблем с логикой поиска по имени функции.)
Создан неуникальный индекс по Content.name (для ускорения поиска по имени)
